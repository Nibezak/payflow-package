...
class ValidateCartForOrderCreation extends BaseValidator
{
    ...
    public function validate(): bool
    {
        ...
        if ($cart->completedOrder) {
            return $this->fail('cart', __('payflow::exceptions.carts.order_exists'));
        }

        // Do we have a billing address?
        if (! $cart->billingAddress) {
            return $this->fail('cart', __('payflow::exceptions.carts.billing_missing'));
        }
        ...
        if ($cart->isShippable()) {
            ...
            if (! $shippingOption = $cart->getShippingOption()) {
                return $this->fail('cart', __('payflow::exceptions.carts.shipping_option_missing'));
            }

            // Is this cart going to be shipped and if so, does it have a shipping address?
            if (! $shippingOption->collect) {
                if (! $cart->shippingAddress) {
                    return $this->fail('cart', __('payflow::exceptions.carts.shipping_missing'));
                }
