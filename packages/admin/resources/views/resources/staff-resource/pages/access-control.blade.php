        ...
        <x-filament-tables::table>
            ...
            @foreach ($this->groupedPermissions as $groupedPermission)
                <x-filament-tables::row>
                    ...
                    @foreach($this->roles as $role)
                        <x-filament-tables::cell
                            :wire:key="'payflow.permission.' . $role->handle . '.' . $groupedPermission->handle"
                            class="!p-0"
                        >
                            <div 
                                x-cloak
                            ...
                            >
                                <x-heroicon-s-check-circle 
                                    x-show="enabled"
                                    :wire:key="'payflow.permission.' . $role->handle . '.' . $groupedPermission->handle.'.enabled'"
                                ...
                                <x-heroicon-s-x-circle 
                                    x-show="!enabled"
                                    :wire:key="'payflow.permission.' . $role->handle . '.' . $groupedPermission->handle.'.disabled'"
                ...
                @foreach($groupedPermission->children as $permission)
                    <x-filament-tables::row>
                        ...
                        @foreach($this->roles as $role)
                            <x-filament-tables::cell
                                :wire:key="'payflow.permission.' . $role->handle . '.' . $groupedPermission->handle . '.' . $permission->handle"
                                class="!p-0"
                            >
                                <div 
                                    x-cloak
                                ...
                                >
                                    <x-heroicon-s-check-circle 
                                        x-show="enabled"
                                        :wire:key="'payflow.permission.' . $role->handle . '.' . $groupedPermission->handle . '.' . $permission->handle.'.enabled'"
                                        class="w-8 h-8 group-hover:scale-110 transition-transform text-green-500 group-hover:text-green-600 dark:text-green-400/80 dark:group-hover:text-green-400"
                                        defer/>

                                    <x-heroicon-s-x-circle 
                                        x-show="!enabled"
                                        :wire:key="'payflow.permission.' . $role->handle . '.' . $groupedPermission->handle . '.' . $permission->handle.'.disabled'"
                                        class="w-8 h-8 group-hover:scale-110 transition-transform text-red-500 group-hover:text-red-600 dark:text-red-400/80 dark:group-hover:text-red-400" 
                                        x-bind:class="{
            ...
            <x-filament-tables::row>
                ...
                @foreach($this->roles as $role)
                    <x-filament-tables::cell
                        :wire:key="'payflow.role.' . $role->handle . '.delete'"
                        class="!p-0"
                    >
                        @if($role->firstParty)
