{{-- <div class="relative group" x-sortable-item="{{ $node['id'] }}">
    <div @class([ 'w-full flex justify-between' , 'dark:bg-gray-900 dark:border-white/10' , ])>
        <div class="flex w-full space-x-2">
            <div @class([ 'flex items-center text-gray-400 hover:text-gray-500' , 'cursor-grab' , ]) x-sortable-handle>
                <x-filament::icon alias="payflow::reorder" class="w-5 h-5" />
            </div>

            <div class="flex grow bg-white border shadow-sm rounded">
                @if($node['children_count'])
                <button class="px-3 text-gray-500 appearance-none" type="button" title=""
                    wire:click.prevent="toggleChildren">
                    <x-filament::icon alias="payflow::chevron-right"
                        @class([ 'w-3.5 h-3.5 transition ease-in-out duration-200 rtl:rotate-180'
                ...
                <button @class([ 'w-full py-2 text-left rtl:text-right appearance-none' , 'px-4'=> false,
                    'cursor-default' => true,
                    ])
                    type="button"
                    >
