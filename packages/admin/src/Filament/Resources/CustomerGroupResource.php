...
use Payflow\Admin\Filament\Resources\CustomerGroupResource\Pages;
use Payflow\Admin\Support\Forms\Components\Attributes;
use Payflow\Admin\Support\Resources\BaseResource;
use Payflow\Models\Contracts\CustomerGroup;
...
class CustomerGroupResource extends BaseResource
{
    ...
    {
        return __('payflowpanel::customergroup.label');
    }

    public static function getPluralLabel(): string
    {
        return __('payflowpanel::customergroup.plural_label');
    }

    public static function getNavigationIcon(): ?string
    {
        return FilamentIcon::resolve('payflow::customer-groups');
    }
    ...
    protected static function getNameFormComponent(): Component
    {
        return Forms\Components\TextInput::make('name')
            ->label(__('payflowpanel::customergroup.form.name.label'))
            ->required()
            ->maxLength(255)
            ->autofocus();
    }
    ...
    protected static function getHandleFormComponent(): Component
    {
        return Forms\Components\TextInput::make('handle')
            ->label(__('payflowpanel::customergroup.form.handle.label'))
            ->required()
    ...
    protected static function getDefaultFormComponent(): Component
    {
        return Forms\Components\Toggle::make('default')
            ->label(__('payflowpanel::customergroup.form.default.label'));
    }
    ...
    protected static function getTableColumns(): array
    {
        return [
            BadgeableColumn::make('name')
                ...
                ->suffixBadges([
                    Badge::make('default')
                        ->label(__('payflowpanel::customergroup.table.default.label'))
                        ->color('gray')
                        ->visible(fn (Model $record) => $record->default),
                ])
                ->label(__('payflowpanel::customergroup.table.name.label')),
            Tables\Columns\TextColumn::make('handle')
                ->label(__('payflowpanel::customergroup.table.handle.label')),
        ];
    }
