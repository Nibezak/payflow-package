...
class RemapShippingPolymorphicRelations extends Migration
{
    public function up()
    {
        \Illuminate\Support\Facades\DB::table($this->prefix.'prices')
            ->where('priceable_type', '=', \Payflow\Shipping\Models\ShippingRate::class)
            ->update([
                'priceable_type' => 'shipping_rate',
            ]);

        \Illuminate\Support\Facades\DB::table($this->prefix.'shipping_exclusions')
            ->where('purchasable_type', '=', \Payflow\Models\Product::class)
            ->update([
    ...
    public function down()
    {
        \Illuminate\Support\Facades\DB::table($this->prefix.'prices')
            ...
            ->update([
                'priceable_type' => \Payflow\Shipping\Models\ShippingRate::class,
            ]);

        \Illuminate\Support\Facades\DB::table($this->prefix.'shipping_exclusions')
            ->where('purchasable_type', '=', 'product')
            ->update([
                'purchasable_type' => \Payflow\Models\Product::class,
            ]);
    }
}
